...
function updatePositions() {
  ...
  // Capture the scroll top adjustment we will use to set the phase.
  var scrollTopAdjust = document.body.scrollTop / 1250;
  var phase;
  // Loop on the moving pizza elements and adjust their horizontal position.
  for (var i = 0; i < moving_pizzas.length; i++) {
    phase = Math.sin(scrollTopAdjust + (i % 5));
...
// Array to store the moving pizza elements for movement during scrolling.
var moving_pizzas = [];
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 200; i++) {
    ...
    // Store the moving pizza element.
    moving_pizzas.push(elem);
  }
  updatePositions();
});
